...
class SignUpActivity : AppCompatActivity() {
    ...
    private fun setupViewModel() {
        ...
        authViewModel.apply {
            ...
            errorCode.observe(this@SignUpActivity) { code ->
                if (code == null)
                    return@observe

                when (code) {
                    409 -> Toast.makeText(this@SignUpActivity, "이미 존재하는 아이디입니다.", Toast.LENGTH_SHORT).show()
                    else -> Toast.makeText(this@SignUpActivity, "서버 오류가 발생했습니다.", Toast.LENGTH_SHORT).show()
                }
            }
        }
